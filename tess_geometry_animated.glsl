...
float shouldDivide(int depth, float lumi) {
    if (depth > maxDepth)
        return 0;
    if (vertices > MAX_VERTICES * 0.9)
        return 0; // stop subdividing
    if (alwaysDivide)
        return 1000;
...
void tessFromEdges(TriangleDivision tri, float perc) {
    // find edge midpoints
    vec4 ab = tri.a + (tri.b - tri.a) * perc;
    vec4 bc = tri.b + (tri.c - tri.b) * perc;
    vec4 ca = tri.c + (tri.a - tri.c) * perc;
...
void main() {
    ...
    while (stackPointer >= 0 && justInCase < 1024) {
        ...
        float perc = shouldDivide(gen, bri);
